...
import { Quiz, Category, Tag } from '../types';

const QuizzesPage: React.FC = () => {
  const { quizzes, categories, tags, fetchQuizzes, fetchCategories, fetchTags } = useQuizStore();
    ...
    <Layout>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div id="filters-modal" className="mt-4 bg-white border border-gray-200 rounded-lg p-4 shadow-lg hidden">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-900 mb-3">Difficulty</h3>
                <div className="flex flex-wrap gap-2">
                  {tags.filter(tag => ['Beginner', 'Intermediate', 'Advanced'].includes(tag.name)).map(tag => (
                    <button
                      key={tag.id}
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-3 mt-6">Topics</h3>
                <div className="flex flex-wrap gap-2">
                  {tags.filter(tag => !['Beginner', 'Intermediate', 'Advanced'].includes(tag.name)).map(tag => (
                    <button
                      key={tag.id}
      ...
      {filteredQuizzes.length > 0 && (
        <section className="py-12 bg-gray-50">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">Featured Quiz</h2>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {filteredQuizzes.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {filteredQuizzes.slice(searchQuery || selectedCategory || selectedTag ? 0 : 1).map(quiz => (
